...
(function (root) {
  ...
  const EXTENSION_ID = "bsen975-circuit-simulation";
  ...
  const TRANSLATION_PREFIX = `${EXTENSION_ID}.`;
  ...
  class LogicCoreController {
    ...
    stop() {
      ...
      this._failAll(new CircuitError("电路模拟器已关闭", "The circuit simulation has stopped"));
      this.worker = null;
      this.localSimulator = null;
      this.localQueue = Promise.resolve();
    }
    ...
    async call(method, ...args) {
      if (!this.running) {
        throw new CircuitError("电路模拟器未启动", "The circuit simulation is not running");
      }
    ...
    _handleWorkerError(worker, event) {
      ...
      this._failAll(new CircuitError(
        ...
        event.message || "circuit simulation worker error"
      ));
    }
  ...
  class Bsen975LogicGateExtension {
    ...
    getInfo() {
      ...
      return {
        id: EXTENSION_ID,
        name: t("name", "circuit simulation"),
        ...
        blocks: [
          ...
          { opcode: "startCore", blockType: command, text: t("block.start", "start circuit simulation") },
          { opcode: "stopCore", blockType: command, text: t("block.stop", "stop circuit simulation") },
          { opcode: "isCoreRunning", blockType: boolean, text: t("block.running", "circuit simulation running?") },
    ...
    openUserGuide() {
      ...
      const existing = document.getElementById("bsen975-circuit-simulation-guide");
      if (existing) existing.remove();

      const t = this._t;
      const dialog = document.createElement("dialog");
      dialog.id = "bsen975-circuit-simulation-guide";
      dialog.setAttribute("aria-labelledby", "bsen975-circuit-simulation-guide-title");
      ...
      style.textContent = [
        "#bsen975-circuit-simulation-guide::backdrop{background:rgba(19,31,42,.58)}",
        "#bsen975-circuit-simulation-guide button:focus-visible{outline:3px solid #3b82b8;outline-offset:2px}",
        "#bsen975-circuit-simulation-guide code{font-family:ui-monospace,SFMono-Regular,Consolas,monospace}"
      ].join("");
      ...
      title.id = "bsen975-circuit-simulation-guide-title";
      title.textContent = t("help.title", "circuit simulation user guide");
      ...
      addList([
        t("help.step1", "Start circuit simulation, then clear the current circuit."),
      ...
      addTable([
        ...
        [t("block.start", "start circuit simulation"), local("创建计算引擎。", "Create the simulation engine."), local("启动 Worker；不支持时使用本地引擎。重复执行不会清空电路。", "Starts a worker or local fallback. Calling it again does not clear the circuit.")],
        [t("block.stop", "stop circuit simulation"), local("释放计算引擎。", "Release the simulation engine."), local("删除运行中的电路和角色绑定，并终止 Worker。", "Deletes the running circuit and bindings, then terminates the worker.")],
        [t("block.running", "circuit simulation running?"), local("检查引擎状态。", "Check engine availability."), local("返回布尔值，不修改状态。", "Returns a Boolean without changing state.")],
  ...
  root.tempExt = {
    ...
    l10n: {
      "zh-cn": {
        [`${EXTENSION_ID}.name`]: "电路模拟器",
        [`${EXTENSION_ID}.description`]: "搭建、运行和验证使用命名线路的数字逻辑电路。"
      },
      en: {
        [`${EXTENSION_ID}.name`]: "circuit simulation",
